...
import type {
  ...
  GameDefs, Observations, AllData, MultiBodyOverrides, DataVersion,
} from './types';
...
export async function loadAllData(): Promise<AllData> {
  ...
  // multiple body_type slots in the optimizer. data-version.json is optional
  // too — surfaced in the footer/README (#255). Trailer prices come straight
  // from game-defs.json: the parser reads them off the dealer presets.
  const [gameDefs, observations, multiBody, dataVersion] = await Promise.all([
