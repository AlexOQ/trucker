...
DLC counts below are the trailer-brand / cargo-pack / map-expansion DLCs each game's bundled data covers. Game version, refresh date, and notes live in `public/data/<game>/data-version.json`; the counts are derived from the `dlc` registry in `game-defs.json`. Regenerate this table with `npx tsx scripts/gen-data-coverage.ts` after a data refresh.

<!-- BEGIN data-coverage (generated by scripts/gen-data-coverage.ts — do not edit by hand) -->
| Game | Version | Refreshed | DLCs (trailer / cargo / map) | Notes |
|------|---------|-----------|------------------------------|-------|
| ETS2 | 1.60.1.0 | 2026-06-18 | 27 (8 / 9 / 10) | — |
| ATS | 1.60.1.0 | 2026-06-18 | 27 (2 / 8 / 17) | Truck cabin/paint catalog not yet populated for ATS. Map-expansion cargo limited to installed states; Special Transport cargo excluded (player-only). |
<!-- END data-coverage -->
