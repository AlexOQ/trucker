...
8. Re-run per game on every game update or DLC — full reseed, no incremental merge needed (a partial-ownership tree reseeds with `--keep-cities`, see the parser header)
...
**Pricing is def-derived, exact to the dealer screen** (2026-09-04, replaced the hand-walked `manual-prices.json`). A dealer preset (`def/vehicle/trailer_dealer`, `def/vehicle/truck_dealer`) lists every accessory on every unit of the combination and the sum of those parts' `price` fields is the sticker price; paint prices sit in `@include`d settings files, so `readSiiWithIncludes()` inlines includes before reading. Non-preset trailer configurations (`trailer_owned/<brand>/configurations/`) are priced by *walking* from the brand's presets the way the upgrade shop does — keep fitting accessories, swap chassis/body, re-count wheels per axle, add new `defaults[]`, fill `require[]` with the cheapest suitable part — and taking the cheapest walk. Verified to the euro on 9 owned ETS2 combinations plus a truck. Trucks additionally carry `kit_price` (interior, wheels, mirrors… the dealer charges on every build) and their `presets`; `computeMinCost` adds the kit so the trucks page matches the dealer. `--prices-only` re-prices an existing `game-defs.json` from a partial def tree without touching anything else; `--keep-cities` is the full-reparse counterpart for that tree (see the parser header).
...
- **Map expansion DLCs (ATS)**: each US state shipped as a separate DLC. Mapping lives in `ATS_STATE_TO_DLC` (`Record<stateCode, dlcId | null>` — `null` is base game; a state absent from the map fails the parse). City→DLC map built dynamically by `buildAtsCityDlcMap(cities)` from each city's `country` (state code) field — no hand-curated `ATS_CITY_DLC_MAP`.
...
- **Cargo pack DLCs (ATS)**: `ATS_CARGO_DLC_MAP` is populated (61 cargo across 7 packs), sourced from each pack's `def/cargo.<dlc>.sii` aggregator inside the owned `dlc_*.scs` archives and cross-checked against `game-defs.json` (#243). Special Transport (`dlc_oversize`) maps to zero cargo — all its cargo is oversize/player-only and excluded from the AI-haulable dataset. `ATS_MAP_DLC_CARGO` is **permanently empty**: ATS state map DLCs ship no cargo defs at all — verified on 1.60.1.8 against 13 owned state archives (AR CO ID MT NM OK OR TX UT WA WY + free AZ NV), none of which contains a `def/cargo` tree or a `def/cargo.dlc_<state>.sii` aggregator. State DLCs extend where existing cargo spawns, never what cargo exists (unlike ETS2 map expansions, which do add shadow cargo). Do not re-open on the next state purchase.
...
- **Company names (ATS)**: localized via the def `name` field by `buildCompanyNameMap` on reparse (#267, #289); all 217 ATS companies resolve, none falls back to `formatCompanyName(id)`.
- **ATS def dump scope**: a def tree extracted from an ATS install carries all trailer, cargo, truck and company defs (brand/pack DLC content included) — only **cities from unowned map DLCs and the company `editor/` placements in them are missing**, which is what trips the `GARAGE_CITIES` drift guard on a plain full reparse. `--keep-cities` is the reparse for that tree: everything the tree has is taken fresh, the missing cities/placements carry forward from the bundled file, and `--diff --keep-cities` shows only the real changes. Bundled `game-defs.json` deliberately keeps all 284 cities across all 20 states; never write it from a partial-ownership extract without `--keep-cities`. `buildAtsCityDlcMap` throws on a state missing from `ATS_STATE_TO_DLC` — `south_dakota` already ships a `country/` def with no cities, and when its DLC lands the state goes into both `ATS_STATE_TO_DLC` and `ATS_MAP_DLCS`.
