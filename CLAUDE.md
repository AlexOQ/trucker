...
- **Company names (ATS)**: localized via the def `name` field for ETS2 only as of #267. ATS company names remain mechanical title-case of the id until an ATS reparse against a complete def dump (the donated ATS defs are partial — a full reparse trips the `GARAGE_CITIES` drift guard). Parser fix is game-agnostic; `buildCompanyNameMap` populates ATS names automatically on that reparse.
