...
/src/frontend       - TypeScript source (compiled by Vite)
  main.ts           - rankings page orchestrator: init, routing, shared state
  rankings-view.ts  - rankings table rendering, search/filter, country dropdown, score tiers
  city-detail-view.ts - city detail rendering, fleet display, export buttons, garage toggle
