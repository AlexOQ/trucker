...
- **Trucks page (ATS)**: cabin and paint data only populated for ETS2 as of #252. Switching to ATS on `trucks.html` renders an empty list. The truck cabin/paint defs are present in the dump (see *ATS def dump scope* below), so either an ATS reparse or a targeted backfill (the #289 pattern) would populate it; the parser is game-agnostic.
