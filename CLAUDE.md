...
- **Company names (ATS)**: localized via the def `name` field by `buildCompanyNameMap` on reparse (ETS2 #267; ATS since the 1.61.1.4 `--keep-cities` reparse, 2026-09-17 — all 217 resolve, including the `aport_*`/`port_*` terminals that the #289 backfill couldn't). `scripts/backfill-company-names.cjs` is retired.
