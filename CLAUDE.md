...
- **ATS def dump scope**: a def tree extracted from an ATS install carries all trailer, cargo, and company defs (brand/pack DLC content included) — only **cities from unowned map DLCs are missing**, which is the single reason a *full* ATS reparse trips the `GARAGE_CITIES` drift guard. Everything-but-cities is therefore recoverable without a full reparse, via targeted backfills keyed by id (the #267 names / #250 axles / #289 names pattern). Confirmed 2026-08-23 against a 14-state install: `--diff` reported 196 changes, every one an unowned-state city/company removal, and **zero** cargo/trailer/truck drift — the bundled ATS data matches a real 1.60.1.8 install exactly. Bundled `game-defs.json` deliberately keeps all 284 cities across all 20 states; never overwrite it from a partial-ownership extract.
